<?php
	class Task_model extends CI_Model
	{
		...
		public function updateTask($taskID,$progress,$error,$clarifications,$status)
		{	
			$sql = "UPDATE task SET progress = ?, error = ?, clarifications = ?,status = ? WHERE taskID = ?";
			//$sql = "INSERT into task (progress,error,clarifications,status) VALUES (?,?,?,?)";
			$this->db->query($sql, array($progress,$error,$clarifications,$status, $taskID));
